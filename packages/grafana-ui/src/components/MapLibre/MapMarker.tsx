...
import { createListeners, pickHandlers } from './utils';

type BaseMapMarkerProps = {
  lngLat: LngLatLike;
} & MaplibreGL.MarkerOptions;

export type MapMarkerProps = Partial<OnMapEventHandlers<BaseMapMarkerProps>> & BaseMapMarkerProps;
  ...
  useMaplibreUIEffect(
    ({ map, maplibre }) => {
      ...
      if (children) {
        ...
        ReactDOM.render(<React.Fragment>{children}</React.Fragment>, el.current);
      }
      const marker = new maplibre.Marker(markerOptions).setLngLat(lngLat).addTo(map);
